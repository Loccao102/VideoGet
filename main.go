...
func main() {
	...
	if err != nil { log.Fatalf("load embedded web UI: %v", err) }
	providers := []source.Provider{source.NewBilibiliProvider(), source.NewDouyinProvider()}
	jobs := download.NewManager(downloadDir)
	api := httpapi.New(providers, jobs, web)
	server := &http.Server{Addr: addr, Handler: api.Handler(), ReadHeaderTimeout: 10*time.Second, ReadTimeout: 30*time.Second, WriteTimeout: 90*time.Second, IdleTimeout: 120*time.Second}
