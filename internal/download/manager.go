...
func (m *Manager) run(id string) {
	...
	sourceOutput, err := m.download(ctx, job.Video)
	if err != nil {
		m.update(id, func(job *Job) {
...
func (m *Manager) download(ctx context.Context, video model.Video) (string, error) {
	switch strings.ToLower(video.Platform) {
	case "bilibili":
		return m.ytdlp(ctx, video.URL)
	case "douyin":
		return m.douyin(ctx, video.URL)
	default:
		return "", fmt.Errorf("unsupported platform %q", video.Platform)
	}
}

func (m *Manager) ytdlp(ctx context.Context, url string) (string, error) {
	bin, err := exec.LookPath("yt-dlp")
	if err != nil {
		return "", fmt.Errorf("yt-dlp is not installed: %w", err)
	}

	template := filepath.Join(m.downloadDir, "%(title).120B [%(id)s].%(ext)s")
...
func (m *Manager) douyin(ctx context.Context, url string) (string, error) {
	...
	cmd := exec.CommandContext(ctx, bin, "-u", url, "-t", "aweme", "-p", m.downloadDir)
	cmd.Env = os.Environ()
	...
	output, err := newestMediaFile(m.downloadDir, startedAt)
	if err != nil {
